...
template <typename T> void PrepareAlloc(T **x, int size, int init = -1) {
  srand(12);
  T *buf = new T[size];
...
template <typename T, typename U> struct IvarOp {
  const U *cache_mean;
  U epsilon;
  int D;
  int C = -1; // C==-1 indicating NCD
...
template <typename T, typename U> struct DvarOp {
  const U *gamma;
  const T *x;
  const U *cache_ivar;
  const U *cache_mean;
...
template <typename T, typename U> struct DmeanOp {
  const U *gamma;
  const T *x;
  const U *cache_ivar;
  const U *cache_mean;
...
template <typename T, typename U> struct DxOp {
  const T *x;
  const U *cache_mean;
  const U *cache_ivar;
  const U *gamma;
...
template <typename T, typename U> struct YOp {
  const U *cache_mean;
  const U *cache_ivar;
  const U *gamma;
  const U *beta;
...
void InstanceNormGradGPU(const T *dy, const T *x, const U *cache_mean,
  ...
  // } else
  {
    ...
    dim3 threads;
    if (is_channel_first) {
      const int reduced_rows = DivUp(N * D, min_rows_per_block * min_rows_per_block);
      ...
      printf("XLOG: NDC, threads dimXYZ=%d,%d,%d\n", threads.x, threads.y,
             threads.z);
      total_tmp_rows = blocks_per_row * thd_y;
  ...
  //   printf("Run CUDA with %d blocks and each %d threads!\n", NxC,
  //   kBlockSize); InstanceNormRowReduceInToOut<<<NxC, kBlockSize>>>(
  ...
  InstanceNormUpdate<<<DivUp(NxC * D, kBlockSize * min_work_per_thread),
                       kBlockSize>>>(dy, NxC * D, D, dx, dx_ops,
                                     is_channel_first);
  cudaEventRecord(stop);
...
__global__ void InstanceNormRowReduceInToTemp(const T *__restrict__ x,
                                               const int N, const int C,
                                               const int D,
...
int main(int argc, char **argv) {
  ...
  if (do_test) overwrite(cache_mean, cache_ivar, cache_mean_cpu, cache_ivar_cpu, N * C);
